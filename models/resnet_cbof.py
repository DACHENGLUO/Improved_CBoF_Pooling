...
class Cos_BoF_SPP_Pooling(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, H, W = quantized_features.shape
        h_mid = int(H/2)
        w_mid = int(W/2)
        histogram1 = torch.mean(quantized_features[:, :, :h_mid, :w_mid], (2, 3))
        histogram2 = torch.mean(quantized_features[:, :, h_mid:, :w_mid], (2, 3))
        histogram3 = torch.mean(quantized_features[:, :, :h_mid, w_mid:], (2, 3))
        histogram4 = torch.mean(quantized_features[:, :, h_mid:, w_mid:], (2, 3))
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.avgpool = Cos_BoF_Pooling(512 * block.expansion,512 * block.expansion)
...
def resnet_cbof18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet_cbof34(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet_cbof50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet_cbof101(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def resnet_cbof152(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
